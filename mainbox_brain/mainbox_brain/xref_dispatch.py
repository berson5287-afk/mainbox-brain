...
__version__ = "0.2.0"  # route input through xref_common.parse_query (natural language + voice-ready); shared normalize
...
def _parse_number_spec(spec: str, max_n: int) -> list[int]:
    """Parse a selection like '2,3' or '1-3' or 'all' into a list of ints.
    Returns [] on 'all' meaning 'everything' is handled by the caller."""
...
def apply_correction(action: str, spec: str, db: str | None = None) -> int:
    """Apply confirm/reject to findings selected by number (or 'all') from the
    last lookup. action is 'confirm' or 'reject'. Returns count applied."""
    ...
    picks = _parse_number_spec(spec, max_n)
    if not picks:
    ...
    for n in picks:
        ...
        try:
            ...
            # xr.confirm/reject print their own count; tally our own too
            n_rows = res if isinstance(res, int) else 1
            applied += 1
...
def _main(argv: list[str]) -> int:
    ...
    if xc is not None:
        ...
        if args.parts[0].lower() in ("confirm", "reject"):
            ...
            if not spec:
                print(f"Usage: {action} <numbers|all>")
                return 2
