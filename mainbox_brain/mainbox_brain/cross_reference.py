...
__version__ = "0.2.1"  # family-tolerant mfr matching (topaz == topaz/southwire) in graph edges
...
def _edges(conn, np: str, cm: str) -> list[tuple]:
    ...
    a: list = [np]
    if cm:
        # family-tolerant: 'topaz' matches 'topaz/southwire' and vice versa
        q += (" AND (src_mfr='' OR src_mfr=? COLLATE NOCASE "
              "OR src_mfr LIKE ? COLLATE NOCASE "
              "OR ? LIKE src_mfr || '%' COLLATE NOCASE)")
        a.extend([cm, cm + "%", cm])
    ...
    a2: list = [np]
    if cm:
        q2 += (" AND (equiv_mfr='' OR equiv_mfr=? COLLATE NOCASE "
               "OR equiv_mfr LIKE ? COLLATE NOCASE "
               "OR ? LIKE equiv_mfr || '%' COLLATE NOCASE)")
        a2.extend([cm, cm + "%", cm])
